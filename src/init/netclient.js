...
function netclient(server, secret, engine_dir) { 
  ...
  io.on('connection',function(socket){
    ...
    socketioJwt.authorize({
      secret: secret,
      timeout: 24*60*60*1000 // 15 seconds to send the authentication message 
    })(socket);
    ...
    socket.on("initserver", function(serverstr, callback) { 
      ...
      let handler = {
        set: function(target, key, value, receiver) {
          ...
          if ((type(value) === '[object Object]' || type(value) === '[object Array]')) {
            if (!isProxy(value)) {
              let pxyObj = setProxyForObj(value, netKey);
              realValue = new Proxy(pxyObj, handler);
            } else {
              //说明对象的这个值是一个proxy
              //说明是对象自己的内部变动，不推送
              //这种情况一般都出现在数组的操作上
      ...
      function setProxyForObj(obj, parentNetKey) {
        ...
        for (let key in obj) {
          ...
          if (type(value) === '[object Object]' || type(value) === '[object Array]') {
            if (!isProxy(value)) {
              let pxyObj = setProxyForObj(value, tempNetKey);
              resObj[key] = new Proxy(pxyObj, handler);
            } else {
              let pxyObj = setProxyForObj(value, tempNetKey);
              obj[key] = pxyObj;
            }
